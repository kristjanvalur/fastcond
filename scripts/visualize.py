...
try:
    ...
    print(
        "Warning: matplotlib not available, skipping chart generation", file=sys.stderr
    )
...
def create_comparison_table(results):
    ...
    for bench_name, bench_results in by_benchmark.items():
        ...
        if bench_results:
            output.append(
                f"**Configuration:** {bench_results[0].get('description', 'N/A')}\n"
            )
        ...
        for result in bench_results:
            ...
            weighted_avg = (
                sum(t["avg_latency_sec"] * t["items"] for t in threads) / total_items
                if total_items > 0
                else 0
            )
            ...
            output.append(
                f"| {impl} | {weighted_avg * 1e6:.2f} | {min_lat * 1e6:.2f} | {max_lat * 1e6:.2f} | {avg_stdev * 1e6:.2f} |\n"
            )
...
def create_throughput_chart(results, output_path):
    ...
    for idx, (bench_name, bench_results) in enumerate(sorted(by_benchmark.items())):
        ...
        for result in sorted(
            bench_results,
            key=lambda x: x["results"]["overall"].get("throughput_items_per_sec", 0),
        ):
            impl = result["implementation"]
            throughput = result["results"]["overall"].get("throughput_items_per_sec", 0)
    ...
    if os.path.exists(output_path):
        print(
            f"📈 Throughput chart saved: {output_path} ({os.path.getsize(output_path)} bytes)"
        )
    else:
...
def create_latency_chart(results, output_path):
    ...
    if os.path.exists(output_path):
        print(
            f"📊 Latency chart saved: {output_path} ({os.path.getsize(output_path)} bytes)"
        )
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize fastcond performance benchmarks"
    )
