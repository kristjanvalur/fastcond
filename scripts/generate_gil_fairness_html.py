    ...
    for result in results:
        ...
        if fairness_mode in modes:
            ...
            modes[fairness_mode] = {
                ...
                "consecutive_percentage": fairness_stats.get(
...
"""
    ...
    for mode_key, mode_data in modes.items():
        ...
        total_acquisitions = (
            sum(t["acquisitions"] for t in acquisitions) if acquisitions else 1
        )
...
"""
        ...
        cv_class = (
            ...
            if cv < 1.0
            else "metric-terrible"
        )

        fairness_score = mode_data["fairness_score"]
        fs_class = (
            "metric-excellent"
            if fairness_score < 10
            else "metric-poor"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate GIL fairness analysis HTML page"
    )
    parser.add_argument("json_file", help="JSON file with GIL benchmark results")
    parser.add_argument(
        "--output", "-o", default="gil-fairness.html", help="Output HTML file"
    )
