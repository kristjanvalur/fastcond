...
def create_fairness_comparison_chart(fair_data, unfair_data, output_dir):
    ...
    fig.suptitle('GIL Fairness Analysis: Fair vs Unfair Implementation', fontsize=16, fontweight='bold')
    ...
    if fair_data['thread_acquisitions'] and unfair_data['thread_acquisitions']:
        ...
        ax1.bar(x - width/2, fair_acq, width, label='Fair GIL', color='skyblue', alpha=0.8)
        ax1.bar(x + width/2, unfair_acq, width, label='Unfair GIL', color='lightcoral', alpha=0.8)
    ...
    metrics = ['Coefficient of\nVariation', 'Consecutive\nRe-acquisitions (%)', 'Fairness\nScore']
    fair_values = [fair_data['coefficient_of_variation'], 
                   fair_data['consecutive_percentage'], 
                   fair_data['fairness_score']]
    unfair_values = [unfair_data['coefficient_of_variation'], 
    ...
    ax2.bar(x - width/2, fair_values, width, label='Fair GIL', color='skyblue', alpha=0.8)
    ax2.bar(x + width/2, unfair_values, width, label='Unfair GIL', color='lightcoral', alpha=0.8)
    ...
    for i, (fair_val, unfair_val) in enumerate(zip(fair_values, unfair_values)):
        ax2.text(i - width/2, fair_val + max(fair_values + unfair_values) * 0.01, 
                f'{fair_val:.3f}', ha='center', va='bottom', fontsize=9)
        ax2.text(i + width/2, unfair_val + max(fair_values + unfair_values) * 0.01, 
    ...
    ax3.bar(x - width/2, fair_perf_norm, width, label='Fair GIL', color='skyblue', alpha=0.8)
    ax3.bar(x + width/2, unfair_perf_norm, width, label='Unfair GIL', color='lightcoral', alpha=0.8)
    ...
    ax3.text(0 - width/2, fair_perf_norm[0] + 5, f'{fair_data["operations_per_sec"]:,}', 
             ha='center', va='bottom', fontsize=9, rotation=90)
    ax3.text(0 + width/2, unfair_perf_norm[0] + 5, f'{unfair_data["operations_per_sec"]:,}', 
             ha='center', va='bottom', fontsize=9, rotation=90)
    ax3.text(1 - width/2, fair_perf_norm[1] + 5, f'{fair_data["avg_latency_us"]:.1f}μs', 
             ha='center', va='bottom', fontsize=9, rotation=90)
    ax3.text(1 + width/2, unfair_perf_norm[1] + 5, f'{unfair_data["avg_latency_us"]:.1f}μs', 
             ha='center', va='bottom', fontsize=9, rotation=90)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate GIL fairness comparison charts')
    ...
    try:
        ...
        if not fair_data or not unfair_data:
            print("Error: Could not find both fair and unfair GIL test results", file=sys.stderr)
            sys.exit(1)
        
        # Generate visualization
        chart_path = create_fairness_comparison_chart(fair_data, unfair_data, output_dir)
        
        print(f"GIL fairness analysis complete!")
